...
describe('WompiService', () => {
  ...
  describe('tokenizeCard', () => {
    it('should return token id on success', async () => {
      ...
      jest
        .spyOn(global, 'fetch')
        .mockResolvedValue(mockFetchResponse(mockResponse) as any);
    ...
    it('should throw CustomException when API returns error', async () => {
      ...
      jest
        .spyOn(global, 'fetch')
        .mockResolvedValue(mockFetchResponse(errorResponse, false, 400) as any);
  ...
  describe('getAcceptanceToken', () => {
    ...
    it('should throw when API response is not ok', async () => {
      jest
        .spyOn(global, 'fetch')
        .mockResolvedValue(mockFetchResponse({}, false, 500) as any);
  ...
  describe('createTransaction', () => {
    ...
    it('should throw CustomException when transaction fails', async () => {
      ...
      jest
        .spyOn(global, 'fetch')
        .mockResolvedValue(mockFetchResponse(errorResponse, false, 422) as any);

      await expect(
        service.createTransaction(mockWompiTransactionData),
      ).rejects.toThrow();
    });
  ...
  describe('getTransaction', () => {
    ...
    it('should throw CustomException when transaction not found', async () => {
      jest
        .spyOn(global, 'fetch')
        .mockResolvedValue(mockFetchResponse({}, false, 404) as any);
  ...
  describe('waitForTransactionStatus', () => {
    ...
    it('should retry while PENDING and return when status changes', async () => {
      ...
      const approvedResponse = {
        data: { id: 'wompi-001', status: 'APPROVED' },
      };
    ...
    it('should return last PENDING response after max retries', async () => {
      ...
      jest
        .spyOn(global, 'fetch')
        .mockResolvedValue(mockFetchResponse(pendingResponse) as any);
